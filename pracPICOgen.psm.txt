      ...
      main_program: LOAD s5, 10                         ;Line 1 position 0
                    CALL LCD_cursor
                    CALL disp_line1	        ;Display a message
			  LOAD s5, 20                         ;Line 2 position 0
                    CALL LCD_cursor
                    CALL disp_line2	        ;Display a message

                    CALL delay_1s;
			
    check_switches: INPUT s0,DATA_IN_PORT     		;get status of switches
			  COMPARE s0, 00    			;test to see if switches are 0
			  JUMP Z, just_clear_the_display 	;if the switches are 0, then just clear the display
		
			  CALL LCD_shift_left               ;if the switches are not 0, then shift the display left 
     			  CALL delay_1s;
			  JUMP check_switches;
